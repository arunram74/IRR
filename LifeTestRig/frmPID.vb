Public Class frmPID
    ...
    Private Sub frmPID_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        txtMV.Text = (CurrentHead.PLC.GetTagVal("PIDMV") / 16000) * 100
    End Sub
    ...
    Private Sub Tmr_Tick(sender As Object, e As EventArgs) Handles Tmr.Tick
        PIDManual = CurrentHead.PLC.GetTagVal("PIDManual")
        If PIDManual Then
        ...
        txtLoad.Text = CurrentHead.PLC.GetTagVal("PIDLoad")
        txtPress.Text = CurrentHead.PLC.GetTagVal("PIDPress")
        txtSV.Text = CurrentHead.PLC.GetTagValUint16("PIDSV")
        txtPV.Text = CurrentHead.PLC.GetTagValUint16("PIDPV")

        If Not txtMV.Enabled Then txtMV.Text = (CurrentHead.PLC.GetTagVal("PIDMV") / 16000) * 100

        If Not txtLoadMul.Enabled Then txtLoadMul.Text = CurrentHead.PLC.GetTagVal("LoadMul")

        If Not txtProp.Enabled Then txtProp.Text = CurrentHead.PLC.GetTagVal("PIDProp")
        If Not txtProp.Enabled Then txtInteg.Text = CurrentHead.PLC.GetTagVal("PIDInt")
        If Not txtProp.Enabled Then txtDiffGain.Text = CurrentHead.PLC.GetTagVal("PIDDiffGain")
        If Not txtProp.Enabled Then txtDiffTime.Text = CurrentHead.PLC.GetTagVal("PIDDiffTime")
        If Not txtProp.Enabled Then txtFilter.Text = CurrentHead.PLC.GetTagVal("PIDFilt")

    End Sub

    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles btnAutoMan.Click
        PIDManual = CurrentHead.PLC.GetTagVal("PIDManual")
        If PIDManual Then CurrentHead.PLC.SetTagVal("PIDManual", 0) Else CurrentHead.PLC.SetTagVal("PIDManual", 1)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        txtFilter.Enabled = False
        chkLoadMul.Checked = False
        CurrentHead.PLC.SetTagVal("PIDProp", If(txtProp.Text <> "", CInt(txtProp.Text), 0))
        CurrentHead.PLC.SetTagVal("PIDInt", If(txtInteg.Text <> "", CInt(txtInteg.Text), 0))
        CurrentHead.PLC.SetTagVal("PIDDiffGain", If(txtDiffGain.Text <> "", CInt(txtDiffGain.Text), 0))
        CurrentHead.PLC.SetTagVal("PIDDiffTime", If(txtDiffTime.Text <> "", CInt(txtDiffTime.Text), 0))
        CurrentHead.PLC.SetTagVal("PIDFilt", If(txtFilter.Text <> "", CInt(txtFilter.Text), 0))
        CurrentHead.PLC.SetTagVal("PIDReset", 1)
    End Sub
    ...
    Private Sub btnMulSave_Click(sender As Object, e As EventArgs) Handles btnMulSave.Click
        CurrentHead.PLC.SetTagVal("LoadMul", Convert.ToSingle(txtLoadMul.Text))
    ...
    Private Sub txtMV_TextChanged(sender As Object, e As EventArgs) Handles txtMV.TextChanged
        ...
        CurrentHead.PLC.SetTagVal("PIDMV", val)
    End Sub
End Class
